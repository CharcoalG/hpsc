...
__global__ void proc(double *Dx_d, double *Dy_d, double *Dt_d, double *ap_d, double *Sm_prime_d, double *Sm_star_d,
	double *p_prime_d, double *p_prime_old_d, int *imax_d, int *jmax_d)
{
	...
	int index = (*jmax_d)*threadIdx.y+threadIdx.x;
	...
	*ap_d = (*Dt_d)*(1/(*Dx_d)+1/delmx)+(*Dt_d)*(1/(*Dy_d)+1/delmy);
	printf("x = %d, y = %d, Dx = %f, Dy = %f, Dt = %f, delmx = %f, delmy = %f, ap = %f \n",
	 threadIdx.x, threadIdx.y, *Dx_d, *Dy_d, *Dt_d, delmx, delmy, *ap_d);
	// printf("x = %d, y = %d ap = %f \n", threadIdx.x, threadIdx.y, *ap_d);
	Sm_prime_d[index] = (-(*Dt_d)*(
		((p_prime_old_d[index+(*jmax_d)]-p_prime_old_d[index])/(*Dx_d))-
		((p_prime_old_d[index-(*jmax_d)]-p_prime_old_d[index])/delmx))*(*Dy_d))+
		(-(*Dt_d)*(
		((p_prime_old_d[index+1]-p_prime_old_d[index])/(*Dx_d))-
		((p_prime_old_d[index-1]-p_prime_old_d[index])/delmx))*(*Dy_d));
	printf("x = %d, y = %d Sm_prime = %f \n", threadIdx.x, threadIdx.y, Sm_prime_d[index]);
}
...
void navierstokes_semi_explicit(double Re, double conver_crit, double L1, double L2, int imax, int jmax, int cn, int cs, int cw, int ce, double u_vel_north, double u_vel_south, double u_vel_west, double u_vel_east, double v_vel_north, double v_vel_south, double v_vel_west, double v_vel_east, int pn, int ps, int pw, int pe, double p_north, double p_south, double p_west, double p_east)
{
	...
	//CUDA variables
	double *Dx_d, *Dy_d, *Dt_d, *ap_d, *Sm_prime_d, *Sm_star_d, *p_prime_old_d, *p_prime_d;
	int *jmax_d, *imax_d;
	...
	while (unsteadiness > conver_crit)
	{
		...
		while (Max > 0.00000001)
		{
			...
			cudaMemcpy(ap_d, &ap, sizeof(double), cudaMemcpyHostToDevice);
			...
			cudaMemcpy(p_prime_old_d, p_prime_old_iter, size_d, cudaMemcpyHostToDevice);

			proc<<<DimGrid, DimBlock>>>(Dx_d, Dy_d, Dt_d, ap_d, Sm_prime_d, Sm_star_d, p_prime_d, p_prime_old_d, imax_d, jmax_d);


			// cudaMemcpy(&ap, ap_d, sizeof(double), cudaMemcpyDeviceToHost);
