...
void navierstokes_semi_explicit(double Re, double conver_crit, double L1, double L2, int imax, int jmax, int cn, int cs, int cw, int ce, double u_vel_north, double u_vel_south, double u_vel_west, double u_vel_east, double v_vel_north, double v_vel_south, double v_vel_west, double v_vel_east, int pn, int ps, int pw, int pe, double p_north, double p_south, double p_west, double p_east)
{
	...
	while (unsteadiness > conver_crit)
	{
		...
		__global__ void blah(double *ap_d, double *Dt_d, double *Dx_d, double *Dy_d, double *Sm_prime_d, 
			double *p_prime_d, double *Sm_star_d, double *p_prime_old_d, int jmax, int imax)
		{
			if(threadIdx.x==0 || threadIdx.y == 0 || threadIdx.x == jmax-1 || threadIdx == imax-1){
				return;
			}
			int myid = threadIdx.y * jmax + threadIdx.x;
			double delmx = (i == 1) ? (*Dx_d / 2) : *Dx_d;
			double delmy = (j == 1) ? (*Dy_d / 2) : *Dy_d;
			*ap_d = (*Dt_d)*((1 / (*Dx_d)) + (1 / delmx))*(*Dy_d) + ((1 / (*Dy_d)) + (1 / delmy))*(*Dx_d);
			Sm_prime_d[myid] = (-(*Dt_d)*(((p_prime_old_d[myid+jmax] - p_prime_old_d[myid]) / (*Dx_d)) 
				- ((p_prime_old_d[myid] - p_prime_old_d[myid-jmax]) / delmx))*(*Dy_d)) + (-(*Dt_d)*(((p_prime_old_d[myid+1] 
					- p_prime[myid]) / (*Dy_d)) - ((p_prime_old_d[myid] - p_prime_old_d[myid - 1]) / delmy))*(*Dx_d));
			p_prime_d[myid] -= (1 / (*ap_d))*(Sm_star_d[myid] + Sm_prime_d[myid]);
		} 
		...
		while (Max > 0.00000001)
		{
			...
			blah<<<DimGrid, DimBlock>>>(ap_d, Dt_d, Dx_d, Dy_d, Sm_prime_d, p_prime_d, Sm_star_d, p_prime_old_d, jmax, imax); 
			//bc
			for (j = 0; j < jmax; j++)
			{
				if (pw == 1)
