...
__global__ void blah(double Dt, double Dx, double Dy, double *Sm_prime_d, double *p_prime_d, 
	double *Sm_star_d, double *p_prime_old_d, int jmax, int imax)
{
	if(threadIdx.x==0 ){
		return;
	}
	...
	if(threadIdx.x == jmax-1){
		return;
	}
	if(threadIdx.y == imax-1){
		return;
	}
	int myid = threadIdx.y * jmax + threadIdx.x;
	double delmx = (threadIdx.x == 0) ? (Dx / 2) : Dx;
	double delmy = (threadIdx.y == 0) ? (Dy / 2) : Dy;
	double ap = Dt*(((1 / Dx) + (1 / delmx))*Dy + ((1 / Dy) + (1 / delmy))*Dx);
	Sm_prime_d[myid] = (-Dt*(((p_prime_old_d[myid+jmax] - p_prime_old_d[myid]) / Dx) - ((p_prime_old_d[myid] - p_prime_old_d[myid-jmax]) / delmx))*Dy) + (-Dt*(((p_prime_old_d[myid+1] - p_prime_old_d[myid]) / Dy) - ((p_prime_old_d[myid] - p_prime_old_d[myid-1]) / delmy))*Dx);
	p_prime_d[myid] -= (1 / ap)*(Sm_star_d[myid] + Sm_prime_d[myid]);
} 
...
void navierstokes_semi_explicit(double Re, double conver_crit, double L1, double L2, int imax, int jmax, int cn, int cs, int cw, int ce, double u_vel_north, double u_vel_south, double u_vel_west, double u_vel_east, double v_vel_north, double v_vel_south, double v_vel_west, double v_vel_east, int pn, int ps, int pw, int pe, double p_north, double p_south, double p_west, double p_east)
{
	...
	double *Sm_prime_d, *p_prime_d, *Sm_star_d, *p_prime_old_d;
	// int size_double = sizeof(double);
	int size_arr = imax*jmax*sizeof(double);
	cudaMalloc(&Sm_prime_d, size_arr);
	cudaMalloc(&p_prime_d, size_arr);
	cudaMalloc(&Sm_star_d, size_arr);
	cudaMalloc(&p_prime_old_d, size_arr);
	dim3 DimGrid(1, 1);     
	...
	while (unsteadiness > conver_crit)
	{
		...
		{
			// printf("%0.9f,", Max);
			cudaMemcpy(Sm_prime_d, Sm_prime, size_arr, cudaMemcpyHostToDevice);
			cudaMemcpy(p_prime_d, p_prime, size_arr, cudaMemcpyHostToDevice);
			cudaMemcpy(Sm_star_d, Sm_star, size_arr, cudaMemcpyHostToDevice);
			cudaMemcpy(p_prime_old_d, p_prime_old_iter, size_arr, cudaMemcpyHostToDevice);
			// printf("Sm00 prev %f .\n", Sm_prime[0][0]);

			blah<<<DimGrid, DimBlock>>>(Dt, Dx, Dy, Sm_prime_d, p_prime_d, Sm_star_d, p_prime_old_d, jmax, imax);

			// printf("Exiting the cuda. \n");
			cudaMemcpy(Sm_prime, Sm_prime_d, size_arr, cudaMemcpyDeviceToHost);
			cudaMemcpy(p_prime, p_prime_d, size_arr, cudaMemcpyDeviceToHost);
