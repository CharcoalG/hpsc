...
__global__ void proc(double *Dx_d, double *Dy_d, double *Dt_d, double *Sm_prime_d, double *Sm_star_d,
	double *p_prime_d, double *p_prime_old_d, int *imax_d, int *jmax_d)
{
	if(threadIdx.x == 0 || threadIdx.x == *jmax_d-1 || threadIdx.y == 0 || threadIdx.y == *imax_d-1){
		return;
	};
	int index = (*imax_d)*threadIdx.y+threadIdx.x;
	// printf("x = %d, y = %d index = %d \n", threadIdx.x, threadIdx.y, index);
	// double delmx = (i == 1) ? (Dx / 2) : Dx;
	// double delmy = (j == 1) ? (Dy / 2) : Dy;
	double delmx = (threadIdx.x == 1) ? (*Dx_d / 2) : *Dx_d;
	double delmy = (threadIdx.y == 1) ? (*Dy_d / 2) : *Dy_d;
	// printf("x = %d, y = %d delmx = %f delmy = %f Dt = %f \n", threadIdx.x, threadIdx.y, delmx, delmy, *Dt_d);
	// for(int j = 0; j<*jmax_d; j++){
	// 	for(int i = 0; i<*imax_d; i++){
	// 		printf("Sm_star[2,1] = %f \n", Sm_prime_d[j*(*imax_d)+i]);
	// 	}
	// }
	// printf("\n ********************** \n");
	// printf("Sm_star[2,1] = %f \n", Sm_star_d[index]);

	double ap = (*Dt_d)*(1/(*Dx_d)+1/delmx)*(*Dy_d)+(*Dt_d)*(1/(*Dy_d)+1/delmy)*(*Dx_d);
	// printf("x = %d, y = %d, Dx = %f, Dy = %f, Dt = %f, delmx = %f, delmy = %f, ap = %f \n",
	 // threadIdx.x, threadIdx.y, *Dx_d, *Dy_d, *Dt_d, delmx, delmy, ap);
	// printf("x = %d, y = %d ap = %f \n", threadIdx.x, threadIdx.y, ap);
	// printf("x = %d, y = %d, index-jmax = %f, index + jmax = %f, index = %f, index-1 = %f, index+1 = %f \n", threadIdx.x, threadIdx.y,  p_prime_old_d[index-(*jmax_d)],
		// p_prime_old_d[index+(*jmax_d)], p_prime_old_d[index], p_prime_old_d[index-1], p_prime_old_d[index+1]);
	Sm_prime_d[index] = (-(*Dt_d)*(
		((p_prime_old_d[index+(*imax_d)]-p_prime_old_d[index])/(*Dy_d))-
		((p_prime_old_d[index]-p_prime_old_d[index-(*imax_d)])/delmy))*(*Dx_d))+
		(-(*Dt_d)*(
		((p_prime_old_d[index+1]-p_prime_old_d[index])/(*Dx_d))-
		((p_prime_old_d[index]-p_prime_old_d[index-1])/delmx))*(*Dy_d));
	// printf("x = %d, y = %d, ap = %f, Sm_star = %f, Sm_prime = %f \n", threadIdx.x, threadIdx.y, ap, Sm_star_d[index], Sm_prime_d[index]);
	// printf("x = %d, y = %d, p_prime before %f \n", threadIdx.x, threadIdx.y, p_prime_d[index]);
	p_prime_d[index] = p_prime_d[index] - (1 / ap)*(Sm_star_d[index] + Sm_prime_d[index]);
	// printf("x = %d, y = %d, p_prime after %f \n", threadIdx.x, threadIdx.y, p_prime_d[index]);
}
...
void navierstokes_semi_explicit(double Re, double conver_crit, double L1, double L2, int imax, int jmax, int cn, int cs, int cw, int ce, double u_vel_north, double u_vel_south, double u_vel_west, double u_vel_east, double v_vel_north, double v_vel_south, double v_vel_west, double v_vel_east, int pn, int ps, int pw, int pe, double p_north, double p_south, double p_west, double p_east)
{
	...
	double Sm_prime[jmax][imax], Sm_star[jmax][imax];
	// double **Sm_extra;
	// double (*Sm_extra)[jmax] = (double(*)[jmax])malloc(imax * sizeof *Sm_extra);
	// double **Sm_extra;
	// Sm_extra = (double **) malloc(jmax*sizeof(double*));
	// for (int l = 0; l<jmax; l++){
	// 	Sm_extra[l] = (double *) malloc(imax*sizeof(double));
	// }
	// double ** Sm_star;
	...
	double p_prime[jmax][imax], p_prime_old[jmax][imax];
	// double ** p_prime;
	// p_prime = new double*[jmax];
	// for (j = 0; j < jmax; j++)
	// 	p_prime[j] = new double[imax];
	// double ** p_prime_old;
	// p_prime_old = new double*[jmax];
	// for (j = 0; j < jmax; j++)
	// 	p_prime_old[j] = new double[imax];
	...
	for (j = 0; j < jmax; j++)
	{
		...
		p_prime_old[j][0] = p_prime_old[j][1];
		p_prime_old[j][imax - 1] = p_prime_old[j][imax - 2];
	...
	for (i = 0; i < imax; i++)
	{
		...
		p_prime_old[jmax - 1][i] = p_prime_old[jmax - 2][i];
		p_prime_old[0][i] = p_prime_old[1][i];
	...
	//CUDA variables
	double *Dx_d, *Dy_d, *Dt_d, *Sm_prime_d, *Sm_star_d, *p_prime_old_d, *p_prime_d;
	int *jmax_d, *imax_d;
	unsigned long size_d = imax*jmax*sizeof(double);
	cudaMalloc(&Dx_d, sizeof(double));
	cudaMalloc(&Dy_d, sizeof(double));
	cudaMalloc(&Dt_d, sizeof(double));
	cudaMalloc(&Sm_prime_d, size_d);
	cudaMalloc(&Sm_star_d, size_d);
	cudaMalloc(&p_prime_d, size_d);
	cudaMalloc(&p_prime_old_d, size_d);
	cudaMalloc(&jmax_d, sizeof(int));
	cudaMalloc(&imax_d, sizeof(int));
	cudaMemcpy(imax_d, &imax, sizeof(int), cudaMemcpyHostToDevice);
	cudaMemcpy(jmax_d, &jmax, sizeof(int), cudaMemcpyHostToDevice);
	dim3 DimGrid(1, 1);     
	dim3 DimBlock(imax, jmax);
	...
	while (unsteadiness > conver_crit)
	{
		...
		while (Max > 0.00000001)
		{
			// printf("%0.9f,", Max);
			double ap = Dt;
			// printf("p_prime_old(2,1) = %f \n", p_prime_old[2][1]);
			//double *Dx_d, *Dy_d, *Dt_d, *Sm_prime_d, *Sm_star_d, *p_prime_old_d, *p_prime_d;
			// printf(" Outer Sm_star[2,1] = %f \n", Sm_star[2][1]);
			// memcpy(Sm_extra, Sm_star, sizeof Sm_star);
			// for (int l = 0; l<jmax; l++){
			// 	memcpy(Sm_extra[l], Sm_prime[l], imax*sizeof(double));
			// }
			// printf(" Outer Sm_extra[2,1] = %f \n", Sm_extra[2][1]);
			cudaMemcpy(Dx_d, &Dx, sizeof(double), cudaMemcpyHostToDevice);
			cudaMemcpy(Dy_d, &Dy, sizeof(double), cudaMemcpyHostToDevice);
			cudaMemcpy(Dt_d, &Dt, sizeof(double), cudaMemcpyHostToDevice);
			cudaMemcpy(Sm_prime_d, Sm_prime, size_d, cudaMemcpyHostToDevice);
			cudaMemcpy(Sm_star_d, Sm_star, size_d, cudaMemcpyHostToDevice);
			cudaMemcpy(p_prime_d, p_prime, size_d, cudaMemcpyHostToDevice);
			cudaMemcpy(p_prime_old_d, p_prime_old, size_d, cudaMemcpyHostToDevice);

			proc<<<DimGrid, DimBlock>>>(Dx_d, Dy_d, Dt_d, Sm_prime_d, Sm_star_d, p_prime_d, p_prime_old_d, imax_d, jmax_d);
			// break;
			cudaMemcpy(Sm_prime, Sm_prime_d, size_d, cudaMemcpyDeviceToHost);
			cudaMemcpy(p_prime, p_prime_d, size_d, cudaMemcpyDeviceToHost);
			// printf("p_prime[2,1] = %f \n", p_prime[2][1]);
			// printf("\n ------------------------------------------------- \n");
			// for (int l = 0; l<jmax; l++){
			// 	for (int m=0; m<imax; m++){
			// 		printf("x = %d, y = %d, Sm_prime = %f, p_prime = %f\n", m, l, Sm_prime[l][m], p_prime[l][m]);
			// 	}
			// }
			// printf ("\n ------------------ \n");
			// for (j = 1; j < jmax - 1; j++)
			...
			// 	for (i = 1; i < imax - 1; i++)
			// 	{
			// 		double delmx = (j == 1) ? (Dx / 2) : Dx;
			// 		double delmy = (i == 1) ? (Dy / 2) : Dy;
			// 		ap = Dt*((1 / Dx) + (1 / delmx))*Dy + Dt*((1 / Dy) + (1 / delmy))*Dx;
			...
			memcpy(p_prime_old, p_prime, jmax*imax*sizeof(double));
		}
