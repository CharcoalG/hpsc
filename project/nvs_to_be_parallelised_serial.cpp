...
void navierstokes_semi_explicit(double Re, double conver_crit, double L1, double L2, int imax, int jmax, int cn, int cs, int cw, int ce, double u_vel_north, double u_vel_south, double u_vel_west, double u_vel_east, double v_vel_north, double v_vel_south, double v_vel_west, double v_vel_east, int pn, int ps, int pw, int pe, double p_north, double p_south, double p_west, double p_east)
{
	...
	double Sm_star[jmax][imax], Sm_prime[jmax][imax];
	// double ** Sm_star;
	...
	while (unsteadiness > conver_crit)
	{
		...
		// printf("p_prime address: %p \n", p_prime);
		// printf("p_prime address: %p \n", p_prime_old);
		// printf("------------------------------------\n");
		// // memcpy(p_prime_old, p_prime, imax*jmax*sizeof(double));
		...
		while (Max > 0.00000001)
		{
...

			{
				for (i = 1; i < imax - 1; i++)
				{
					...
					// printf("x = %d, y = %d, index-jmax = %f, index + jmax = %f, index = %f, index-1 = %f, index+1 = %f \n", j, i, p_prime_old[j][i-1],
						// p_prime_old[j][i+1], p_prime_old[j][i], p_prime_old[j-1][i], p_prime_old[j+1][i]);
					// printf("x = %d, y = %d, Dx = %f, Dy = %f, Dt = %f, delmx = %f, delmy = %f, ap = %f \n", j, i, Dx, Dy, Dt, delmx, delmy, ap);
					// printf("x = %d, y = %d delmx = %f delmy = %f Dt = %f \n", j, i, delmx, delmy, Dt);
					Sm_prime[j][i] = (-Dt*(((p_prime_old[j][i + 1] - p_prime_old[j][i]) / Dx) - ((p_prime_old[j][i] - p_prime_old[j][i - 1]) / delmx))*Dy) + (-Dt*(((p_prime_old[j + 1][i] - p_prime_old[j][i]) / Dy) - ((p_prime_old[j][i] - p_prime_old[j - 1][i]) / delmy))*Dx);
					// printf("x = %d, y = %d Sm_prime = %f  Sm_star = %f\n", j, i, Sm_prime[j][i], Sm_star[j][i]);
					// printf("p_prime before = %f \n", p_prime_old[j][i]);

					p_prime[j][i] = p_prime[j][i] - (1 / ap)*(Sm_star[j][i] + Sm_prime[j][i]);

					// printf("p_prime after = %f \n", p_prime_old[j][i]);
				}
			}
